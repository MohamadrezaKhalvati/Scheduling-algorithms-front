  ...
  <div class="container text-primary relative-position">
    ...
    <div class="container--search center max-container-y mt-md-4 mt-2">
      <q-input v-model="filter.text"
        ...
        clearable
        dense
        class="bg-grey-4 px-1 py-0 shadow-3 container--search--title"
        @keyup.enter="search" />
    ...
    <div class="row my-2">
      <template v-if="(cards && cards.length) && !skeleton">
        <HorizentalCard v-for="card of cards"
          :key="card.id"
          class="col-lg-12 col-md-6 col-xs-12 my-1"
          :skeleton="skeleton"
          :card="card" />
      </template>

      <template v-if="skeleton">
        <HorizentalCard v-for="i of paginationInput.pageSize"
          :key="i"
          class="col-lg-12 col-md-6 col-xs-12 my-1"
          :skeleton="true" />
      </template>
...
<script src="./EventTabContainer.ts" lang="ts" />
