...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        function setDefaultValue(props: object, key: string, defaultProp: any) {
            props[key] = props.hasOwnProperty(key)
                ? props[key]
        ...
        function onValChanged(newValue) {
            ...
            if (shouldBeEnglified(newValue)) eventVal = toEnglish(newValue)
        ...
        function setDefaulProps(fieldValue: FieldValue) {
            ...
            const defaultProp =
                FormConfig.defaultField.default[props.field['component']] || []
            for (const key in defaultProp)
                setDefaultValue(fieldValue, key, defaultProp)

            const extendPropMap =
                FormConfig.defaultField.extend[props.field['component']] || []
            for (const key in extendPropMap) {
                ...
                else fieldValue[key] = extendPropMap[key]
            }
        }
